...
type colorBlindness = 'protanopia' | 'deuteranopia' | 'tritanopia' | 'achromatopsia' | 'invertColors' | 'none'
...
export const useAccessibilityContext = () => {
  ...
  if (!context) {
    throw new Error('useAccessibilityContext must be used within an AccessibilityContextProvider');
  }
  return context;
};

export const AccessibilityContextProvider: React.FC<{ children: React.ReactNode }> = ({
    children,
  }) => {
    ...
    useEffect(() => {
      ...
      localStorage.setItem('isHearingImpaired', JSON.stringify(isHearingImpaired));
    }, [isHearingImpaired]);
